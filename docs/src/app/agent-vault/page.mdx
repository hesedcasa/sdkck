...
Nothing is cached to disk, so no token is ever persisted. The re-executed command does not receive `AGENT_VAULT_TOKEN`; the proxy credential travels inside the proxy URL instead.
