...
- **[Agent Vault](/agent-vault)** — Run commands without giving them real secrets; a vault broker swaps credentials on the wire
- **[Telemetry](/telemetry)** — OpenTelemetry traces and metrics for every command invocation, local and quiet by default
...
2. **MCP server** — Built on the [Model Context Protocol SDK](https://modelcontextprotocol.io/docs/sdk), exposing all commands over stdio for seamless integration.
3. **Permission system** — A glob-based allowlist that controls which commands are visible and executable — the first matching rule wins, and unmatched commands are allowed by default.

## Platforms

Published as an npm package. Runs on macOS, Linux, and Windows wherever **Node.js 22+** (>=22.0.0) is installed.
