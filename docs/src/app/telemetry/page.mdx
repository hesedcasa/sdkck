...
The `setup-telemetry` init hook wraps `Command.prototype._run`, so every command that actually executes is measured — including nested runs triggered through the MCP server or `search`.
...
Print spans and metrics to stdout — useful for a quick look while developing:

```bash
SDKCK_OTEL_CONSOLE=1 sdkck api list
# or
OTEL_TRACES_EXPORTER=console sdkck api list
```
...
| `OTEL_TRACES_EXPORTER=console` | Also selects the console exporter |
...
| `SDKCK_OTEL_CONSOLE` | Export to stdout (`1`/`on`/`true`/`yes`) |
