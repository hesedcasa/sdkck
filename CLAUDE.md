...
- **Fails closed:** if no proxy credential can be resolved — the token is rejected, the vault does not exist, MITM is off — the command does not run at all (exit 1) rather than sending unbrokered requests. `SDKCK_AGENT_VAULT_DISABLED=1` is the escape hatch. `SDKCK_AGENT_VAULT_FALLBACK=1` opts out: the failure is warned about (`Agent Vault unavailable for vault "…": … Running without brokered credentials.`) and the command runs in this process, unbrokered. It applies only to `AgentVaultSetupError` — a failure to spawn the child still exits 1, since the command may already have started. The fallback run does not get `AGENT_VAULT_TOKEN` either — it is deleted from this process's environment first.
