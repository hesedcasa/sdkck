...
- **Token/address resolution:** explicit config > `AGENT_VAULT_TOKEN` / `AGENT_VAULT_ADDR` > `<configDir>/agent-vault.json` (`token`/`address` fields) > `http://localhost:14321` (a missing token throws `AgentVaultError`). The config file is read by `readAgentVaultFileConfig()` (`src/agent-vault/config-file.ts`); `resolveConfigDir()` in the same file replicates oclif's `Config.configDir` resolution (`SDKCK_CONFIG_DIR` > `XDG_CONFIG_HOME`/`LOCALAPPDATA` > `~/.config`, joined with `sdkck`) without instantiating a full oclif `Config`, since this package doubles as a library and a real `Config.load()` would resolve against the *consumer's* package instead. A file that exists but is malformed (not a JSON object, or a non-string field) throws rather than being silently ignored.
